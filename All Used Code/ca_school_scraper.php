<?php
  ...
  $query = "SELECT COUNT(*) as count FROM `canada_pages` WHERE scraped=1";
  ...
  $result->close();
	...
	$query = "SELECT * FROM `canada_pages` WHERE scraped=0 order by rand() LIMIT 5";
	$result = $mysqli->query($query) or die($mysqli->error.__LINE__);
  ...
  $query = "SELECT COUNT(*) as count FROM `canada_pages` WHERE scraped=0";
	$result = $mysqli->query($query) or die($mysqli->error.__LINE__);
  if($row = $result->fetch_assoc()) {
  ...
  if(empty($kill)) {
    //header("refresh:1; ca_school_scraper.php");
    echo "Number of web-pages previously scraped: ".$numcrawl."<br/>\n";
    echo "Number of web-pages scraped this instance: ".$numrow."<br/>\n";
    echo "Number of web-pages scraped in total: ".($numcrawl+$numrow)."<br/>\n";
    echo "Number of web-pages left to scrape: ".$num2crawl."<br/>\n\n";
  } else {
